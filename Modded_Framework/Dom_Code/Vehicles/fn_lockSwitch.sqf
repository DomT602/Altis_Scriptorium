...
if (_vehicle isKindOf "House_F") exitWith {
	if (_vehicle in client_keys || (((_vehicle getVariable ["house_keyHolders",[]]) findIf {getPlayerUID player isEqualTo (_x select 0)}) != -1)) then {
		private _door = 0;
		...
		for "_i" from 1 to _doors do {
			...
			if (player distance _pos < 1.5) exitWith {_door = _i};
		};
